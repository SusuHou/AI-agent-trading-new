...
| 36G | Before allocating the full experiment, measure the exact formal training loop on the target cluster without changing the economic model | Narval one-CPU throughput benchmark / Narval 单 CPU 吞吐率测试 | Fresh paper-mode sandbox; warm-up then exact checkpoint resume; exact period-budget accounting; zero measurement rows; low/high separate processes; environment and checksum receipt; production-root isolation | `CODE AND SLURM WRAPPER IMPLEMENTED; NARVAL RUN PENDING`: the Python tool invokes the real formal session runner, reloads Step-36D's validated status, proves `fresh -> resumed` including the exact checkpoint reference, and refuses convergence or measurement claims. Its general defaults are 1,000 warm-up plus 10,000 timed periods; the first 30-minute Slurm pilot conservatively overrides this to 25+250 per cell, after which the observed rate can justify a longer benchmark. It records pinned Python/NumPy, whitelisted Slurm/thread fields, commit/source identities, training-only and end-to-end rates, RSS diagnostics, execution scope, and an immutable checksum. Local smoke receipts are explicitly distinct and publish no million-period extrapolation. The wrapper runs the full suite from a clean detached clone, launches low/high cells in separate processes, and saves reports under `$SCRATCH`. This is operational evidence only: it does not measure convergence, produce `Delta^C`, or establish paper results. / `代码与 SLURM 包装已实现；尚待 NARVAL 实跑`：Python 默认 1,000+10,000 期，首次 30 分钟 pilot 保守使用每个 cell 25+250 期；真实正式 runner 会核对 fresh、同一 checkpoint 的 resumed、零测量记录、环境/源码身份、执行范围与报告校验。它只提供运行速度证据，不提供收敛或论文结果。 |
...
Orders 6-36G and the formal Step-36 core entrypoint are implemented and validated through their explicitly stated
